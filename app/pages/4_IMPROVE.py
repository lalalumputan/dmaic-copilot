...
# ============================================
_TAB_LABELS = [
    "✏️ Input", "📄 Report", "💡 Solutions & Selection",
    "📋 Implementation", "📢 Communication Plan", "🔄 Should-Be Process",
]
tab_input, tab_report, tab_solutions, tab_impl, tab_comm, tab_should_be = st.tabs(_TAB_LABELS)
...
with tab_input:
    ...
    # Quick path: Step 3 disembunyikan → kolom Solution jadi teks bebas (ketik langsung)
    if _proj_path == "quick" or not _sol_idea_options:
        _sol_col = st.column_config.TextColumn(
            "Solution", required=False, help="Ketik solusi yang akan diimplementasikan",
        )
    else:
        _sol_col = st.column_config.SelectboxColumn(
    ...
    if _proj_path != "quick":
        ...
        if st.button("💾 Simpan Implementation Plan", key=f"impl_save_{active_pid}", disabled=is_locked):
            ...
            # Trigger Step 6: auto-generate communication plan (Step 6 hanya Standard)
            if not is_locked and _proj_path != "quick":
    ...
    if _proj_path != "quick":
        ...
        if _ana_dot_baseline:
            ...
            with st.expander("📊 Lihat Current State Map", expanded=False):
                try:
                    st.graphviz_chart(compact_dot(_ana_dot_baseline), use_container_width=False)
                except Exception:
        ...
        if _cur_imp_dot:
            ...
            try:
                st.graphviz_chart(compact_dot(_cur_imp_dot), use_container_width=False)
            except Exception as _e:
    ...
    if _status_rows:
        ...
        if _all_done:
            st.success("✅ Semua solusi sudah **done** — silakan upload bukti, lalu fase Improve siap di-finalize.")
        else:
    ...
    # ── Bukti / Hasil Implementasi (upload) ──
    st.markdown("#### 📎 Bukti / Hasil Implementasi")
    # Bukti hanya bisa diupload setelah SEMUA implementation status = done
    _impl_done_now = bool(st.session_state.get(_status_key)) and all(
        str(s.get("status", "")).strip().lower() == "done"
        for s in st.session_state.get(_status_key, [])
    )
    if _impl_done_now:
        st.caption("Upload foto/dokumen hasil implementasi di lapangan (PNG/JPG/PDF). Tampil di tab Implementation & Word report.")
    else:
        st.info("📎 Upload bukti aktif **setelah** semua Implementation Status (Step 8) = **done**.")
    _impres_b64_key  = f"improve_implresult_b64_{active_pid}"
    _impres_name_key = f"improve_implresult_name_{active_pid}"
    _impres_mime_key = f"improve_implresult_mime_{active_pid}"
    if _impres_b64_key not in st.session_state:
        _impres_wip = memory.load_improve_wip(active_pid)
        st.session_state[_impres_b64_key]  = _impres_wip.get("implementation_result_b64", "")
        st.session_state[_impres_name_key] = _impres_wip.get("implementation_result_name", "")
        st.session_state[_impres_mime_key] = _impres_wip.get("implementation_result_mime", "")
    _impres_up = st.file_uploader(
        "Upload bukti implementasi", type=["png", "jpg", "jpeg", "pdf"],
        key=f"improve_implresult_uploader_{active_pid}",
        disabled=is_locked or not _impl_done_now,
    )
    if _impres_up is not None:
        import base64 as _b64ir
        _b64s = _b64ir.b64encode(_impres_up.getvalue()).decode("ascii")
        if _b64s != st.session_state.get(_impres_b64_key):
            st.session_state[_impres_b64_key]  = _b64s
            st.session_state[_impres_name_key] = _impres_up.name
            st.session_state[_impres_mime_key] = _impres_up.type or ""
            _w = memory.load_improve_wip(active_pid)
            _w["implementation_result_b64"]  = _b64s
            _w["implementation_result_name"] = _impres_up.name
            _w["implementation_result_mime"] = _impres_up.type or ""
            memory.save_improve_wip(active_pid, _w)
            st.success(f"✅ {_impres_up.name} tersimpan.")
    if st.session_state.get(_impres_b64_key):
        st.caption(f"Tersimpan: {st.session_state.get(_impres_name_key, 'bukti')}")
        if str(st.session_state.get(_impres_mime_key, "")).startswith("image/"):
            import base64 as _b64sh2
            st.image(_b64sh2.b64decode(st.session_state[_impres_b64_key]), width=360)
        if st.button("🗑️ Hapus Bukti", key=f"improve_implresult_del_{active_pid}", disabled=is_locked):
            for _k in (_impres_b64_key, _impres_name_key, _impres_mime_key):
                st.session_state[_k] = ""
            _w = memory.load_improve_wip(active_pid)
            for _wk in ("implementation_result_b64", "implementation_result_name", "implementation_result_mime"):
                _w.pop(_wk, None)
            memory.save_improve_wip(active_pid, _w)
            st.rerun()
    ...
    st.caption("Pastikan Step 1–8 (termasuk Implementation Status & bukti) sudah disimpan, lalu klik Generate. Tidak perlu input tambahan.")
    ...
    if submitted:
        # Quick path: Step 3 (Potential Solutions) disembunyikan — input solusi langsung
        # dari Step 4 (Selected Solution). Potential dibiarkan kosong (gate A2 di-skip di quick).
        user_inputs = {
            "industry":             _industry,
            ...
            "implementation_result_uploaded": bool(
                st.session_state.get(f"improve_implresult_b64_{active_pid}")
                or memory.load_improve_wip(active_pid).get("implementation_result_b64")
            ),
            "pilot_inputs": {
                "scope":            _pilot_scope,
            ...
            "improved_process_map_dot":     (st.session_state.get(_imp_dot_key, "") if _proj_path != "quick" else ""),
...
with tab_report:
    if not improve_state:
        ...
        # Deteksi Step 8 berubah setelah Generate → Report bisa basi
        _live_st  = [(s.get("solution",""), s.get("status","")) for s in (st.session_state.get(_status_key) or [])]
        _draft_st = [(s.get("solution",""), s.get("status","")) for s in (outs.get("implementation_status") or [])]
        if _live_st and _live_st != _draft_st:
            st.warning(
                "⚠️ **Implementation Status (Step 8) berubah setelah Generate.** "
                "Klik **Generate / Update Draft** lagi di tab Input agar Gate, Coaching, & Report ikut ter-update."
            )
        ...
        if _improved_dot_rep or should_be:
            ...
            if _improved_dot_rep:
                ...
                try:
                    st.graphviz_chart(compact_dot(_improved_dot_rep), use_container_width=False)
                except Exception:
                ...
                if _dot:
                    try:
                        st.graphviz_chart(compact_dot(_dot), use_container_width=False)
                    except Exception:
                        pass

        # ── Agent Coaching (paling bawah) — dari draft/final yang sama dengan tab Input ──
...
with tab_solutions:
    if not improve_state:
        ...
        # Must Criteria = B-rule (standard-only); sembunyikan di Quick path
        must_criteria = outs.get("must_criteria") or []
        if _proj_path != "quick" and must_criteria:
...
with tab_impl:
    if not improve_state:
        ...
        # Pilot Plan = B-rule (standard-only); sembunyikan di Quick path
        pilot = outs.get("pilot_plan") or {}
        if _proj_path != "quick" and pilot:
        ...
        # Bukti / Hasil Implementasi (dari upload Step 8)
        _ir_wip = memory.load_improve_wip(active_pid)
        _ir_b64 = st.session_state.get(f"improve_implresult_b64_{active_pid}") or _ir_wip.get("implementation_result_b64", "")
        if _ir_b64:
            import base64 as _b64ti
            st.divider()
            st.markdown("### 📎 Bukti / Hasil Implementasi")
            _ir_name = st.session_state.get(f"improve_implresult_name_{active_pid}") or _ir_wip.get("implementation_result_name", "bukti")
            ...
            if str(_ir_mime).startswith("image/"):
                ...
                st.download_button(
                    ...
                    file_name=_ir_name, mime=_ir_mime or "application/octet-stream",
                    key=f"dl_implresult_{active_pid}",
                )

# ── TAB 5: Communication Plan ──
with tab_comm:
    if _proj_path == "quick":
        st.info("📢 Tab **Communication Plan** tidak berlaku untuk **Quick path** — Step 6 di-skip.")
    elif not improve_state:
...
with tab_should_be:
    if _proj_path == "quick":
        st.info("🔄 Tab **Should-Be / Future State** tidak berlaku untuk **Quick path** — Step 7 di-skip.")
    elif not improve_state:
        ...
        if _sb_baseline:
            with st.expander("📊 Current State Map (baseline dari ANALYZE)", expanded=False):
                try:
                    st.graphviz_chart(compact_dot(_sb_baseline), use_container_width=False)
                except Exception:
        ...
        if _sb_dot:
            st.caption("🟢 Node hijau = baru/berubah setelah improvement")
            try:
                st.graphviz_chart(compact_dot(_sb_dot), use_container_width=False)
            except Exception:
            ...
            if _sb_fallback:
                try:
                    st.graphviz_chart(compact_dot(_sb_fallback), use_container_width=False)
                except Exception:
                    pass
        else:
...
with tab_input:
    ...
    with c1:
        ...
        if _has_draft_now:
            ...
            _has_end = render_phase_end_date(active_pid, "improve", disabled=is_locked)
            _no_actual_end = (_proj_path != "quick") and not _has_end
            fin_disabled = _gate_fail_block or (not _impl_all_done) or _prev_block or _no_actual_end
            ...
            if _gate_fail_block:
                ...
                st.caption("⚠️ Finalize diblokir: fase sebelumnya (ANALYZE) belum di-approve oleh approver (jalur Standard).")
            elif _no_actual_end:
    ...
    if not final_exists:
        ...
        if _prev_approved_ip:
            ...
            with st.expander("↩️ Revisi Improve (buka kembali untuk diedit)", expanded=False):
                st.warning(
                    "Quick path tidak punya approver. Klik tombol di bawah untuk **membuka kembali** "
                    "fase Improve yang sudah final (mis. update Implementation Status). Gate menuju "
                    "Control akan terkunci lagi hingga Improve di-finalize ulang."
                )
                if st.button("↩️ Buka Revisi Improve", key="ip_auto_revise_btn"):
                    st.session_state[_revise_key]     = True
                    st.session_state["improve_draft"] = None
                    audit.log_phase_event(active_pid, "improve", "revise_opened_auto_advance")
                    st.success("Fase Improve dibuka untuk revisi.")
                    st.rerun()
        else:
